...
$add=new DbMysql();
$result=$add->select("select * from mall_articleType ");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="logBackground">
    ...
    <form action="./../deal/articleAddDeal.php" method="post" name="article" id="article" onsubmit="return test();">
        ...
        <div>
            ...
            <input type="text" name="author" style="border: 1px solid rgba(86,61,124,.6);" >
        </div>
        <div>
            <label for="com">文章来源: &nbsp;</label>
            <input type="text" name="com" style="border: 1px solid rgba(86,61,124,.6);" >
        </div>
        <div>
            <label for="hits">浏览数量: &nbsp;</label>
            <input type="text" name="hits" style="border: 1px solid rgba(86,61,124,.6);" >
            可以初始化一个数量，用于展示
        </div>
